...
..#.###...
    ...
    fn example() -> Map<Tile> {
        arrange_tiles(EXAMPLE.trim().split("\n\n").map(|tile_str| {
            tile_str
                .parse()
                .expect("all example tiles parse successfully")
        }))
        .expect("arrangement can be found")
    }
...
...###...##...#...#..###
    ...
    fn example_image() -> Map<Bool> {
        Map::try_from(EXAMPLE_IMAGE.trim()).expect("example image is valid")
    }

    const EXAMPLE_SEA_MONSTERS: &str = "
.####...#####..#...###..
#####..#..#.#.####..#.#.
.#.#...#.###...#.##.O#..
#.O.##.OO#.#.OO.##.OOO##
...
#..###....##.#...##.##.#
    ...
    fn example_sea_monsters() -> Map<Bool> {
        Map::try_from(EXAMPLE_SEA_MONSTERS.trim().replace('O', "#").as_str())
            .expect("example image is valid")
    }

    #[test]
    fn example_produces_example_image() {
        let image = convert_to_image(example());
        let example_image = example_image();

        println!("example image:\n{}", example_image);
        println!("image:\n{}", image);
        println!("orientations:");

        let mut contains_example_image = false;
        for orientation in all_orientations(&image) {
            println!("{}", orientation);
            if orientation == example_image {
                contains_example_image = true;
                break;
            }
        }

        assert!(
            contains_example_image,
            "example image must be generated from example problem"
        );
    }

    #[test]
    fn example_image_has_two_sea_monsters() {
        let example_image = example_sea_monsters();
        assert_eq!(count_sea_monsters_in(&example_image), 2);
    }
}
